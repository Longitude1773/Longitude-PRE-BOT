...
async function main() {
  ...
  if (!url) throw new Error("Usage: tsx scripts/workflows/create-on-demand-eval-manual.ts --url <listing-url> [--channel <id>] [--thread-ts <ts>] [--force-post] [--web-research|--no-web-research] [--dry-run]");
  ...
  const eventPath = await logSlackEvent("on-demand-eval-request-manual", {
    url,
    requestKey,
    channel,
    threadTs,
    ...
    enableWebResearch,
    dryRun,
    recovery: "missing_on_demand_requests_table",
  ...
  if (existingThreadTs && !forcePost) {
    console.log(JSON.stringify({
      ...
      eventPath,
      dryRun,
      recovery: "manual_registry_bypass",
    }, null, 2));
    return;
  }
    ...
    if (!dryRun && resolved) {
      ...
      await upsertThreadContext({
        ...
        event: {
          ...
          note: "Recovered existing on-demand review post from local post ledger via manual registry bypass.",
          status: "posted",
  ...
  if (!dryRun) {
    ...
    if (threadTs) {
      ...
      const postResult = await post(channel, text);
      postedThreadTs = asString((postResult as { ts?: string })?.ts || "");
    }
    ...
    await upsertThreadContext({
      ...
      event: {
        kind: "posted",
        note: "Initial on-demand evaluation review posted to Slack via manual registry bypass.",
        status: "posted",
        version: Number(nextRow.Version),
        eventPath,
      },
